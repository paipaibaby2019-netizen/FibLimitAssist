...
> **多空切换按钮**（v1.07 改）：**水平居中 + 垂直放到线段中点**（`price_mid = (price_1.00 + price_0.00) / 2`）。点击后对调 `1.00`（起点）与 `0.00`（终点），方向自动翻转，`0.79`/`0.49` 回归理论比例。按钮文字实时显示当前方向（`LONG` / `SHORT` / `FLAT`），底色随方向变化（绿 / 红 / 灰）。**点击后立即自动回弹**（不再 sticky）。
...
| 平一半 | `CLOSE HALF` | 最下面那根线的上方，**`CLOSE ALL` 左侧** | 按手数砍半平掉**账户全部品种**的持仓；砍半后 < 品种最小手数则**全平该仓位** |
| 清仓 | `CLOSE ALL` | 最下面那根线的上方（与 0.79/0.49 按钮右对齐） | 一键平掉账户内**全部持仓**，**不涉及挂单** |
...
7. 顶部按钮：`SWAP` 切换多空方向，`CANCEL` 取消全部挂单；底部按钮：风险档位（显示 `0.5%`/`1%`/`2%`）循环切换，`HIDE` 隐藏/显示所有 UI，`BUY MKT`/`SELL MKT` 市价下单（居中），`CLOSE HALF` 平一半，`CLOSE ALL` 全平。
