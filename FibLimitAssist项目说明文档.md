...
> **多空切换按钮**：最上面那根线的上方，**位于 `CANCEL` 按钮左侧**，与 `CANCEL` 紧密相邻。点击后对调 `1.00`（起点）与 `0.00`（终点），方向自动翻转，`0.79`/`0.49` 回归理论比例。按钮文字实时显示当前方向（`LONG` / `SHORT` / `FLAT`），底色随方向变化（绿 / 红 / 灰）。按钮随顶部线移动。
...
### 3.5 功能控制按钮（最下面那根线左侧上方，左对齐）

| 按钮 | 文字 | 作用 |
|------|------|------|
| 隐藏/显示 | `HIDE` / `SHOW` | 切换显示 EA 全部线条与按钮（HIDE 按钮自身始终显示）。状态会话内有效，MT5 重启/EA 重附后恢复显示。 |
| 风险档位 | `RISK 1%` | 循环切换单笔风险档位：`0.5% → 1% → 2% → 0.5%`。当前档位影响**所有下单动作**（0.79/0.49 挂单、市价单）。会话内持久化。 |
| 市价下单 | `BUY MKT` / `SELL MKT` / `--` | 方向为 `LONG` 时显示 `BUY MKT`（绿），`SHORT` 时显示 `SELL MKT`（红），`FLAT` 时显示 `--`（灰）。止损 = `1.00 价 ± Range×1%`，盈亏比 1:1。 |
...
7. 顶部按钮：`SWAP` 切换多空方向，`CANCEL` 取消全部挂单；底部按钮：`RISK 1%` 循环风险档位，`HIDE` 隐藏/显示所有 UI，`CLOSE HALF` 平一半，`CLOSE ALL` 全平。
