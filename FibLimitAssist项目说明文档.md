...
> **STEP 微调按钮**（v1.17 新增）：`1.00` / `0.79` / `0.49` / `0.00` 四条线各有一对 **▲ / ▼** 小按钮（图表左侧，UP 左 / DOWN 右）。单击移动步长 = swing 区间 × `InpStepPercent%`（默认 1%），双击同按钮 300ms 内 = ×10 加速。`1.00`/`0.00` 端点可自由推开挂单线（v1.21 放开）；`0.79`/`0.49` 挂单线不能跨过对方或顶/底端点，撞界时拒绝移动并 PlaySound 反馈。按钮位置按视觉高/低端自动适配（顶线按钮在线下、底线按钮在线上、中间线按钮中心与线对齐，v1.22）。配色浅灰底黑字（v1.24）。
>
> **端点位置记忆**（v1.15）：`1.00`/`0.00`/`0.79`/`0.49` 位置**不再写入全局变量**——每次插入/重启都用当前可见价格区间重新生成默认 fib，仅风险档位（`g_riskPercent`）会话内持久。
...
6. **STEP ▲/▼ 微调**（v1.17）：只移动目标线并保留其他线当前位置（端点变化**不**重置 0.79/0.49）；0.79/0.49 撞界拒绝移动并 PlaySound 反馈；**不改动服务器已有挂单**。
...
2. 端点/挂单线位置**不再记忆**（v1.15）：每次插入/重附都按**当前可见价格区间**重新生成默认斐波那契；仅风险档位（`g_riskPercent`）会话内持久（切换周期/缩放保留）。
...
7. 顶部按钮（最上面线下方 4px）：`SWAP` 切换多空方向、`ADJUST` 一键对齐最近高低点、`CANCEL` 取消全部挂单；底部按钮：风险档位（`0.5%`/`1%`/`2%`）循环切换、`HIDE` 隐藏/显示所有 UI、`BUY MKT`/`SELL MKT` 市价下单（居中，两侧实时盈亏数字）、`EVEN` 一键保本、`CHALF` 平一半、`CALL` 全平；左侧四条主线各一对 STEP ▲/▼ 微调按钮。
