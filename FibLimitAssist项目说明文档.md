...
> **线条水平方向**：5 条线均为**向右延伸的水平射线**，左端默认在图表可见范围中间，右端自动延伸到最右边。左端锚点可**左右拖拽拉伸**（所有线共享同一左边界，一起拉伸），也可上下拖拽改价格（改价逻辑同下）。
>
> **标签位置**：`1.00`、`0.00` 标签锚定于线的左端点，使用 `ANCHOR_RIGHT_LOWER`（文本向左上方展开），位于线的**左上方**；`0.21` 标签为像素定位（`OBJ_LABEL`），右边缘与右侧按钮（CANCEL / 下单按钮）右边缘对齐，位于 `0.21` 线的**上方**。
