...
2. 端点/挂单线位置**按周期记忆**（v1.36）：先尝试用临时全局变量恢复上次位置；首次加载或 MT5 重启后清空时，才按**当前可见价格区间**重新生成默认斐波那契。风险档位（`g_riskPercent`）同样会话内持久（切换周期/缩放保留）。
...
> **版本缺口已回补**：v1.10~v1.36 已全部同步至本文档。其中 v1.10/v1.11/v1.12/v1.14 在 git 中无独立提交（本地迭代后随 v1.13 一次性推送，或被后续版本号跳号），内容以代码注释标注为准。
